...
class Stub:
    ...
    def call(self, app_id: str, data: Any, uid: str = 'super-user') -> dict:
        """
        Sends a request to the specified app via its Remote connection with retry logic.
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    logging.warning(f"[{app_id}] Attempt {attempt + 1} failed: {e}. Retrying...")
                    time.sleep(2 ** attempt)  # Exponential backoff
                    continue
                logging.error(f"[{app_id}] Execution failed after {max_retries} attempts: {e}")
                raise
